...
function OfficerLogin() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axiosInstance.post("/officer/login", form);
      ...
      if (res.data?.data) {
        ...
        id = res.data.data.id || res.data.data.officerId;
      } else if (res.data?.token) {
        token = res.data.token;
        id = res.data.id || res.data.officerId;
      ...
      login({ token, role: "OFFICER", id: id || 1 });
      showToast("success", "Officer login successful");
    } catch (err) {
      console.error(err);
      showToast("error", "Invalid officer credentials");
    }
  };
